...
public class MaxValDiscountPolicyTests {
    ...
    private MaxValDiscountPolicy maxValDiscountPolicy;
    ...
    @ParameterizedTest
    @CsvSource({"200.23,200.10", "1000.5, 205.7", "3000, 13.9"})
    public void calculateDiscount_firstDisIsMax(Double firstDiscountValue, Double secDiscountValue){
        ...
        maxValDiscountPolicy = new MaxValDiscountPolicy(firstDiscountPolicy,secDiscountPolicy);
        Double returnedDiscountVal = maxValDiscountPolicy.calculateDiscount(new ConcurrentHashMap<>());
        Assertions.assertTrue(returnedDiscountVal.equals(firstDiscountValue));

    }

    @ParameterizedTest
    @CsvSource({"200.23,200.99", "1000.5, 2050.7", "3, 13.9"})
    public void calculateDiscount_secDisIsMax(Double firstDiscountValue, Double secDiscountValue){
        Mockito.when(firstDiscountPolicy.calculateDiscount(Mockito.any())).thenReturn(firstDiscountValue);
        Mockito.when(secDiscountPolicy.calculateDiscount(Mockito.any())).thenReturn(secDiscountValue);
        maxValDiscountPolicy = new MaxValDiscountPolicy(firstDiscountPolicy,secDiscountPolicy);
        Double returnedDiscountVal = maxValDiscountPolicy.calculateDiscount(new ConcurrentHashMap<>());
        Assertions.assertTrue(returnedDiscountVal.equals(secDiscountValue));
