...
public interface StoreManagerRepository extends JpaRepository<StoreManager, RolesId> {
    ...
    @Query(value = "SELECT store_name FROM store_manager WHERE member_name = ?",nativeQuery = true)
    public List<String> getStoreNameByMemberName(String memberName);
    @Query(value = "SELECT my_boss_type, my_boss FROM store_manager WHERE member_name = ? AND store_name = ?",nativeQuery = true)
